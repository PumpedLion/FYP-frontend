...
class _NotificationsPageState extends State<NotificationsPage> {
  ...
  void _startTimer() {
    _timer = Timer.periodic(const Duration(minutes: 1), (timer) {
      if (mounted) {
        setState(() {
          // Triggers rebuild to update relative timestamps
        });
      }
    });
  }
  ...
  Map<String, dynamic> _getTheme(String type) {
    switch (type) {
      ...
      case 'FOLLOW':
        return {'category': 'Social', 'icon': Icons.person_add_outlined, 'color': Colors.purple.shade300, 'iconColor': Colors.white};
      case 'SYSTEM':
      default:
