...
import 'FollowedAuthorsPage.dart';
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _initSocket() async {
    ...
    // Listen for real-time notifications to update the dashboard
    SocketService.notifications.listen((notification) {
      if (mounted) {
        _fetchNotifications(); // Refresh notifications and unread count
        _fetchDashboardStats(); // Update stats if necessary
      }
    });

    // Listen for deletions to refresh activity list
    ...
    // Listen for explicit stat updates (reads, follows, etc.)
    SocketService.statsUpdates.listen((update) {
      print('Dashboard: Received real-time stat update: $update');
      if (mounted) {
        _fetchDashboardStats();
      }
    });
  ...
  void _startRefreshTimer() {
    _refreshTimer = Timer.periodic(const Duration(minutes: 1), (timer) {
      if (mounted) {
        setState(() {
          // This triggers a rebuild, which updates the relative 'time' strings
        });
      }
    });
  }
  ...
  void _onPageChanged(int index) {
    ...
    if (index == 2) {
      ...
      // --- IF FOLLOWED AUTHORS CLICKED ---
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => const FollowedAuthorsPage()),
      );
    } else if (index == 7) {
      // --- IF PURCHASE HISTORY CLICKED ---
            ...
            onPressed: () {
              Navigator.push(
                ...
                MaterialPageRoute(
                 ...
                 _fetchNotifications(); // Refresh unwread count when back
              });
            },
            icon: Badge(
...
// --- UPDATED _buildBody ---
  Widget _buildBody(bool isDesktop) {
    switch (_currentIndex) {
      case 0:
        return _dashboardHomeContent(isDesktop);
      case 1:
  ...
  Widget _statsGrid(bool isDesktop) {
    return Wrap(
      ...
      children: [
        ...
        StatCard(
          title: "Followers", 
          value: _totalFollowers.toString(), 
          icon: Icons.people_outline, 
          color: Colors.blue.shade50,
          isLoading: _isLoadingStats,
          onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const FollowersPage())),
        ),
      ],
    );
  }
